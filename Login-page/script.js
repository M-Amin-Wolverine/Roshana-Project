...
(function() {
    ...
    const CONFIG = {
        ...
        api: {
            ...
            weather: 'https://api.open-meteo.com/v1/forecast?latitude={lat}&longitude={lon}&current=temperature_2m,weather_code&timezone=Asia%2FTehran'
        },
        music: {
    ...
    const WeatherManager = {
        ...
        async getLocationAndWeather() {
            ...
            try {
                // Get IP
                const ipData = await fetch(CONFIG.api.ip).then(r => r.json());
                ...
                const weatherUrl = CONFIG.api.weather
                    .replace('{lat}', locationData.latitude)
                    .replace('{lon}', locationData.longitude);
                
                const weatherData = await fetch(weatherUrl).then(r => r.json());
                
                const data = {
                    location: locationData.city || 'تهران',
    ...
    const Bootstrapper = {
        async init() {
            ...
            console.log('%c روشنــا v3.2.5 ', 'background: #00e0ff; color: #000; font-size: 16px; padding: 4px;');
